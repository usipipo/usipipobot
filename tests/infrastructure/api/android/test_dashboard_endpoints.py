...
class TestDashboardSchemas:
    ...
    def test_data_summary_schema(self):
        ...
        data = DataSummaryInfo(
            total_used_bytes=2361393152, total_limit_bytes=5368709120, source="package"
        )
    ...
    def test_dashboard_summary_response_schema(self):
        ...
        response = DashboardSummaryResponse(
            user=UserInfo(telegram_id=123456789, username="testuser", status="active"),
            data_summary=DataSummaryInfo(
                total_used_bytes=2361393152, total_limit_bytes=5368709120, source="free_tier"
            ),
            active_keys=[
                ActiveKeyInfo(
                    id="uuid-1234", name="Mi iPhone", key_type="outline", used_bytes=1288490188
                )
            ],
...
class TestDashboardEndpoint:
    ...
    @pytest.mark.asyncio
    async def test_get_dashboard_summary_success(self, mock_session, mock_current_user):
    ...
    @pytest.mark.asyncio
    async def test_get_dashboard_summary_user_not_found(self, mock_session, mock_current_user):
...
class TestDashboardDataAggregation:
    ...
    def test_package_priority_over_free_tier(self):
        ...
        # With active package
        package_row = MagicMock(data_limit_bytes=16106127360)  # 15 GB
