...
class TestBuyGbHandler:
    ...
    @pytest.mark.asyncio
    async def test_select_payment_method_valid_package(
        self, handler, mock_update, data_package_service
    ):
    ...
    async def test_pay_with_crypto_success(self, handler, mock_update):
        ...
        with patch(
            ...
            order_mock.id = "test-order-id"
            payment_service_mock.create_order.return_value = order_mock

            def get_service_side_effect(service_class):
                if service_class.__name__ == "WalletManagementService":
                    return wallet_service_mock
                elif service_class.__name__ == "CryptoPaymentService":
                    return payment_service_mock
                return MagicMock()
