...
class MenuMixin:
    ...
    async def show_consumption_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            has_pending_debt = (
                summary is not None and not summary.is_active and summary.billing_id is not None
            )
            can_activate, _ = await self.billing_service.can_activate_consumption(user_id, user_id)
