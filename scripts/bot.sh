...
select_target_directory() {
    ...
    if validate_bot_directory "$(pwd)"; then
        ...
        echo ""
        read -r -p "¿Usar este directorio? (S/n): " confirm
    ...
    if [ -z "$PROJECT_DIR" ]; then
        ...
        if [ ${#found_bots[@]} -gt 0 ]; then
            ...
            for i in "${!found_bots[@]}"; do
                local bot_name
                bot_name=$(basename "${found_bots[$i]}")
            ...
            read -r -p "Selecciona un bot (0-${#found_bots[@]}): " selection
    ...
    while [ -z "$PROJECT_DIR" ]; do
        ...
        if validate_bot_directory "$INPUT_DIR"; then
            ...
            echo ""
            read -r -p "¿Intentar con otro directorio? (S/n): " retry
            if [[ "$retry" =~ ^[nN]$ ]]; then
...
create_venv() {
    ...
    if [ -d "$VENV_DIR" ]; then
        ...
        if [ ! -f "$ACTIVATE_SCRIPT" ] || [ ! -f "$PYTHON_BIN" ]; then
            ...
            read -r -p "¿Eliminar y recrear? (S/n): " recreate
            ...
            read -r -p "¿Recrearlo de todos modos? (s/N): " force_recreate
    ...
    if ! $TARGET_PYTHON -m venv "$VENV_DIR"; then
...
install_dependencies() {
    ...
    if "$PIP_BIN" install -r "$REQUIREMENTS_PATH"; then
...
full_install() {
    ...
    if $SUDO apt install -y python3.13 python3.13-venv python3.13-dev python3-pip -qq; then
    ...
    if [ ! -f "$PROJECT_DIR/.env" ]; then
        ...
        read -r -p "¿Configurar ahora? (S/n): " config_env
...
configure_env() {
    ...
    if [ -f "$PROJECT_DIR/.env" ]; then
        ...
        read -r -p "¿Deseas reconfigurarlo? (s/N): " recreate
    ...
    # Solicitar datos
    read -r -p "🔑 TOKEN de Telegram Bot: " TELEGRAM_TOKEN
    read -r -p "👤 ADMIN_CHAT_IDS (separados por coma): " ADMIN_IDS
    read -r -p "🌦️  OpenWeatherMap API Key (Enter para omitir): " WEATHER_KEY
...
ask_and_update_version() {
    ...
    read -r -p "Selecciona una opción (0-3): " v_choice
...
start_bot() {
    ...
    if systemctl is-active --quiet "$SERVICE_NAME" 2>/dev/null; then
        ...
        read -r -p "¿Deseas reiniciarlo? (s/N): " restart_opt
...
update_dependencies() {
    ...
    if install_dependencies; then
        ...
        echo ""
        read -r -p "¿Reiniciar el bot para aplicar cambios? (S/n): " restart_opt
...
remove_dependency() {
    ...
    read -r -p "Número de dependencia a eliminar: " selection
    ...
    if [[ "$selection" =~ ^[0-9]+$ ]] && [ "$selection" -gt 0 ] && [ "$selection" -le "${#lines[@]}" ]; then
        ...
        read -r -p "¿Reiniciar bot? (s/N): " restart_opt
        if [[ "$restart_opt" =~ ^[sS]$ ]]; then
            manage_service "restart"
        fi
...
while true; do
    ...
    case $choice in
        1)  full_install ;;
        2)  create_venv; read -r -p "Presiona Enter para continuar..." ;;
        3)  update_dependencies ;;
        4)  configure_env; read -r -p "Presiona Enter para continuar..." ;;
        5)  create_systemd_service; read -r -p "Presiona Enter para continuar..." ;;
        6)  start_bot ;;
        7)  stop_bot ;;
        8)  restart_bot ;;
        9)  status_bot ;;
