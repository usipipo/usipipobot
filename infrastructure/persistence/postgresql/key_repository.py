...
class PostgresKeyRepository(BasePostgresRepository, IKeyRepository):
    ...
    def _entity_to_model(self, entity: VpnKey) -> VpnKeyModel:
        return VpnKeyModel(
            ...
            name=entity.name, key_data=entity.key_data, external_id=entity.external_id,
            is_active=entity.is_active, used_bytes=entity.used_bytes, last_seen_at=entity.last_seen_at,
            data_limit_bytes=entity.data_limit_bytes, billing_reset_at=entity.billing_reset_at
        )
    ...
    async def save(self, key: VpnKey, current_user_id: int) -> VpnKey:
        ...
        try:
            if key.id:
                ...
                if existing:
                    existing.name, existing.key_data, existing.external_id = key.name, key.key_data, key.external_id
                    existing.is_active, existing.used_bytes, existing.last_seen_at = key.is_active, key.used_bytes, key.last_seen_at
                    existing.data_limit_bytes, existing.billing_reset_at = key.data_limit_bytes, key.billing_reset_at
                else:
    ...
    async def get_by_user_id(self, telegram_id: int, current_user_id: int) -> List[VpnKey]:
        await self._set_current_user(current_user_id)
        try:
            query = select(VpnKeyModel).where(VpnKeyModel.user_id == telegram_id, VpnKeyModel.is_active == True)
    ...
    async def update_usage(self, key_id: uuid.UUID, used_bytes: int, current_user_id: int) -> bool:
        await self._set_current_user(current_user_id)
        try:
            query = update(VpnKeyModel).where(VpnKeyModel.id == key_id).values(
                used_bytes=used_bytes, last_seen_at=datetime.now(timezone.utc))
            await self.session.execute(query)
            await self.session.commit()
            return True
        except Exception as e:
    ...
    async def update_data_limit(self, key_id: uuid.UUID, data_limit_bytes: int, current_user_id: int) -> bool:
        await self._set_current_user(current_user_id)
        try:
            query = update(VpnKeyModel).where(VpnKeyModel.id == key_id).values(data_limit_bytes=data_limit_bytes)
            await self.session.execute(query)
            await self.session.commit()
            return True
        except Exception as e:
    ...
    async def reset_data_usage(self, key_id: uuid.UUID, current_user_id: int) -> bool:
        ...
        try:
            query = update(VpnKeyModel).where(VpnKeyModel.id == key_id).values(
                used_bytes=0, billing_reset_at=datetime.now(timezone.utc))
            await self.session.execute(query)
            await self.session.commit()
            return True
        except Exception as e:
    ...
    async def get_keys_needing_reset(self, current_user_id: int) -> List[VpnKey]:
        ...
        try:
            ...
            query = select(VpnKeyModel).where(
                VpnKeyModel.billing_reset_at < thirty_days_ago, VpnKeyModel.is_active == True)
            result = await self.session.execute(query)
            return [self._model_to_entity(m) for m in result.scalars().all()]
        except Exception as e:
