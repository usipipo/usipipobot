...
class PostgresDataPackageRepository(BasePostgresRepository, IDataPackageRepository):
    ...
    def _entity_to_model(self, entity: DataPackage) -> DataPackageModel:
        return DataPackageModel(
            ...
            package_type=(
                ...
                if isinstance(entity.package_type, PackageType)
                else entity.package_type
            ),
            ...
            if data_package.id:
                ...
                if existing:
                    existing.package_type = (
                        data_package.package_type.value
                        if isinstance(data_package.package_type, PackageType)
                        else data_package.package_type
                    )
    ...
    async def get_expired_packages(self, current_user_id: int) -> List[DataPackage]:
        ...
        try:
            now = datetime.now(timezone.utc)
            query = select(DataPackageModel).where(
                DataPackageModel.is_active == True, DataPackageModel.expires_at < now
            )
            result = await self.session.execute(query)
            return [self._model_to_entity(m) for m in result.scalars().all()]
        except Exception as e:
